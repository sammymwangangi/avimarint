...
var require_consola = __commonJS({
  "node_modules/consola/dist/consola.js"(exports, module) {
    ...
    var e = __require("path");
    var t = __require("fs");
    var r = u(__require("os"));
    ...
    var Gu = typeof __non_webpack_require__ != "undefined" ? __non_webpack_require__ : __require;
    var zu = class {
      constructor(u2) {
        if (u2 && u2.log)
          this.logger = u2;
...
// src/constants.ts
var NAME = "nuxt-delay-hydration";
var CONFIG_KEY = "delayHydration";
var MODE_DELAY_APP_INIT = "init";
var MODE_DELAY_APP_MOUNT = "mount";
var MODE_DELAY_MANUAL = "manual";

// src/util/template.ts
var _fs = require('fs');

var _escapeRegExp = require('lodash/escapeRegExp'); var _escapeRegExp2 = _interopRequireDefault(_escapeRegExp);
var templateUtils = (options = {}) => {
  options = __spreadValues({
    publishPath: _upath.join.call(void 0, _upath.dirname.call(void 0, __dirname), ".runtime")
  }, options);
  const template = (r) => {
    let content = "";
    const contents = () => {
      if (content)
        return content;
      content = _fs.readFileSync.call(void 0, r.src, { encoding: "utf-8" });
      return content;
    };
    const injectFileContents = (file, afterLine) => {
      ...
      const templateToInject = _fs.readFileSync.call(void 0, file, { encoding: "utf-8" });
      const subst = `$1
${templateToInject}`;
      const regex = new RegExp(`(${_escapeRegExp2.default.call(void 0, afterLine)})`, "gm");
      content = content.replace(regex, subst);
      return originalContent !== content;
    };
    const publish = () => {
      const newPath = _upath.join.call(void 0, options.publishPath, _upath.basename.call(void 0, r.src));
      _fs.writeFileSync.call(void 0, newPath, content);
      r.custom = true;
      r.originalSrc = r.src;
      r.src = newPath;
      return r;
  ...
  const matchTemplate = (templates, id) => {
    const match = templates.find((template2) => template2.src.endsWith(_upath.join.call(void 0, "vue-app", "template", `${id}.js`)));
    if (!match)
      return null;
...
var template_default = templateUtils;

// src/logger.ts
var import_consola = __toModule(require_consola());
var logger_default = import_consola.default.withScope(`nuxt:${NAME}`);

// src/index.ts
var nuxtDelayHydration = _kit.defineNuxtModule.call(void 0, (nuxt) => ({
  name: NAME,
  ...
  setup: (config) => {
    var _a;
    if (!config.mode) {
      logger_default.info(`\`${NAME}\` mode set to \`${config.mode}\`, disabling module.`);
      return;
    }
    if (!nuxt.options.ssr) {
      logger_default.warn(`\`${NAME}\` will only work for SSR apps, disabling module.`);
      return;
    }
    if (nuxt.options.vite && !((_a = nuxt.options.vite) == null ? void 0 : _a.ssr)) {
      logger_default.warn(`\`${NAME}\` only works with vite with SSR enabled, disabling module.`);
      return;
    }
    if (!config.debug && nuxt.options.dev) {
      logger_default.info(`\`${NAME}\` only runs in dev with \`debug\` enabled, disabling module.`);
      return;
    }
    if (config.debug && !nuxt.options.dev)
      logger_default.warn(`\`${NAME}\` debug enabled in a non-development environment.`);
    if (nuxt.options.target !== "static")
      logger_default.warn(`\`${NAME}\` is untested in a non-static mode, use with caution.`);
    nuxt.hook("build:before", () => {
      if (process.env.NODE_ENV !== "test")
        logger_default.info(`\`${NAME}\` enabled with \`${config.mode}\` mode ${config.debug ? "[Debug enabled]" : ""}`);
      nuxt.options.render.asyncScripts = true;
    });
    const delayHydrationPath = _upath.join.call(void 0, "hydration", "hydrationRace.js");
    const replayPointerEventPath = _upath.join.call(void 0, "hydration", "replayPointerEvent.js");
    _kit.addTemplate.call(void 0, {
      src: _upath.join.call(void 0, _upath.resolve.call(void 0, __dirname, "runtime", "template", "delayHydration.js")),
      fileName: delayHydrationPath,
      options: config
    });
    if (config.replayClick) {
      _kit.addTemplate.call(void 0, {
        src: _upath.resolve.call(void 0, _upath.join.call(void 0, __dirname, "runtime", "template", "replayPointerEvent.js")),
    ...
    nuxt.hook("components:dirs", (dirs) => {
      dirs.push({
        path: _upath.resolve.call(void 0, _upath.join.call(void 0, __dirname, "components")),
        ignore: ["index.js"]
      });
    });
    if (config.mode === MODE_DELAY_MANUAL) {
      _kit.addPlugin.call(void 0, {
        src: _upath.resolve.call(void 0, _upath.join.call(void 0, __dirname, "runtime", "plugin", "injectDelayHydrationApi.js")),
        fileName: _upath.join.call(void 0, "hydration", "pluginDelayHydration.client.js"),
        options: config
      });
    }
    const utils = template_default({ publishPath: _upath.join.call(void 0, _upath.dirname.call(void 0, __dirname), ".runtime") });
    if (config.mode === MODE_DELAY_APP_INIT || config.mode === MODE_DELAY_APP_MOUNT) {
      nuxt.hook("build:templates", ({ templateVars, templatesFiles }) => {
        if (config.mode === MODE_DELAY_APP_MOUNT) {
          ...
          template.injectFileContents(_upath.join.call(void 0, __dirname, "runtime", "templateInjects", "import.js"), "import Vue from 'vue'");
          template.injectFileContents(_upath.join.call(void 0, __dirname, "runtime", "templateInjects", "delayHydrationRace.js"), "async function mountApp (__app) {");
          template.publish();
          return;
        }
        if (config.mode === MODE_DELAY_APP_INIT) {
          ...
          templateVars.hydrationConfig = config;
          template.injectFileContents(_upath.join.call(void 0, __dirname, "runtime", "templateInjects", "import.js"), "import Vue from 'vue'");
          template.injectFileContents(_upath.join.call(void 0, __dirname, "runtime", "templateInjects", "delayHydrationRace.js"), "async function createApp(ssrContext, config = {}) {");
          template.publish();
        }
      });
    }
...
var src_default = nuxtDelayHydration;
