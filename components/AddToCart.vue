<template>
  <b-modal :id="`${product.url}`" :ref="`${product.url}`" :title="`${activeProduct.name}`"
    ...
    <div class="row pb-2">
      ...
      <div class="col-8 col-xs-6 col-sm-8 col-md-8 col-lg-8">
        ...
        <button class="btn btn-danger btn-sm rounded-0 float-right" type="button"
                @click="$bvModal.hide(`${product.url}`)"><i class="ri-close-fill" style="position: relative;top:2px;"></i>close
        </button>
        </div>
      </div><!-- end image column-->
    </div>
